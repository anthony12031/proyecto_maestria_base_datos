...
BEGIN 
	FOR estudiante in EXECUTE 'SELECT * FROM estudiantes' LOOP
		EXECUTE FORMAT ('CREATE USER MAPPING FOR "%s" SERVER ciencias_educacion OPTIONS (user %L, password %L)',estudiante.cod_e,estudiante.cod_e,estudiante.cod_e);
		EXECUTE FORMAT ('GRANT USAGE ON FOREIGN SERVER ciencias_educacion TO "%s"',estudiante.cod_e);
	END LOOP;
END;
$$ LANGUAGE plpgsql;
...
BEGIN 
	FOR profesor in EXECUTE 'SELECT * FROM profesores' LOOP
		EXECUTE FORMAT ('CREATE USER MAPPING FOR "%s" SERVER ciencias_educacion OPTIONS (user %L, password %L)',profesor.id_p,profesor.id_p,profesor.id_p);
		EXECUTE FORMAT ('GRANT USAGE ON FOREIGN SERVER ciencias_educacion TO "%s"',profesor.id_p);
	END LOOP;
END;
$$ LANGUAGE plpgsql;
...
BEGIN 
	FOR coordinador in EXECUTE 'SELECT * FROM carreras where id_carr <> 10001' LOOP
		EXECUTE FORMAT ('CREATE USER MAPPING FOR "%s" SERVER ciencias_educacion OPTIONS (user %L, password %L)',coordinador.id_carr,coordinador.id_carr,coordinador.id_carr);
		EXECUTE FORMAT ('GRANT USAGE ON FOREIGN SERVER ciencias_educacion TO "%s"',coordinador.id_carr);
	END LOOP;
END;
$$ LANGUAGE plpgsql;
...
SELECT dblink_connect('ciencias_educacion conn', 'ciencias_educacion');

-- consulta a ciencias
SELECT * FROM dblink('myconn','SELECT n1 FROM ciencias_educacion.inscribe') AS t(n1 float);